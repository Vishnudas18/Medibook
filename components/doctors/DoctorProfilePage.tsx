...
import { ApiResponse, IDoctorProfileWithUser, TimeSlot } from '@/types';
...
export default function DoctorProfilePage() {
  ...
  const { data: doctor, isLoading, isError } = useQuery({
    queryKey: ['doctor', id],
    queryFn: async () => {
      const { data } = await axios.get<ApiResponse<IDoctorProfileWithUser>>(`/api/doctors/${id}`);
      if (!data.success || !data.data) throw new Error(data.error || 'Failed to fetch doctor');
      return data.data;
    },
    enabled: Boolean(id),
  });
  ...
  return (
    <div className="container mx-auto py-10 space-y-8 pb-20">
      <button
        onClick={() => router.back()}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
        <div className="lg:col-span-2 space-y-8">
              ...
              <div className="flex-1 space-y-4">
                ...
                <div className="flex flex-wrap items-center justify-center md:justify-start gap-x-6 gap-y-3">
                  ...
                  <div className="flex items-center gap-2 text-slate-600 font-semibold bg-amber-50/50 px-3 py-1 rounded-xl border border-amber-100/50 shadow-sm">
                    ...
                    <span className="text-slate-400 text-xs font-medium border-l border-slate-200 pl-2">
                ...
                <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
                  {doctor.languages.map((lang) => (
                    <Badge
                      key={lang}
                      variant="outline"
                      className="px-3 py-1 rounded-lg border-slate-200 bg-slate-50/50 text-slate-600 font-medium"
                    >
                      {lang}
                    </Badge>
                  ))}
          ...
          <section className="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm space-y-6">
            ...
            <p className="text-slate-600 leading-relaxed text-lg italic">"{doctor.about}"</p>
                ...
                <div className="flex items-start gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {doctor.qualifications.map((q) => (
                      <Badge key={q} className="bg-white text-slate-700 border-slate-200 font-semibold">
                        {q}
                      </Badge>
                    ))}
                  </div>
          ...
          <section className="bg-white rounded-3xl p-8 border border-slate-200 shadow-xl overflow-hidden">
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="text-right">
                <p className="text-[10px] items-center text-slate-400 uppercase tracking-widest font-bold mb-1">
                  Fee
                </p>
            ...
            {selectedSlot && (
              <div className="mt-8 space-y-6 animate-fade-in border-t border-slate-50 pt-8">
                <SymptomSelector specialization={doctor.specialization} onChange={setReason} />
          ...
          {!user && (
            <div className="p-4 rounded-2xl bg-amber-50 border border-amber-100 flex gap-3">
              ...
              <p className="text-xs text-amber-800 leading-relaxed font-medium">
                ...
                as a patient to book this slot.
              </p>
            </div>
          )}
        </div>
      ...
      {selectedSlot && selectedDate && isBookingModalOpen && (
        <BookingModal
