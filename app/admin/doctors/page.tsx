    ...
    <div className="space-y-10">
      ...
      <Tabs defaultValue="pending" onValueChange={setActiveTab} className="w-full">
        <TabsList className="bg-white/50 border border-slate-100 p-1.5 rounded-2xl h-14 w-full flex justify-start sm:w-auto mb-8 shadow-sm">
          <TabsTrigger value="pending" className="rounded-xl px-10 font-bold flex gap-2">
            Pending Approval <div className="bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full text-[10px]">{pendingDoctors.length}</div>
          </TabsTrigger>
          <TabsTrigger value="approved" className="rounded-xl px-10 font-bold flex gap-2">
            Active <div className="bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full text-[10px]">{approvedDoctors.length}</div>
          </TabsTrigger>
          <TabsTrigger value="rejected" className="rounded-xl px-10 font-bold flex gap-2">
          ...
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow className="border-slate-100 hover:bg-transparent">
                <TableHead className="py-5 font-bold text-slate-400 uppercase text-[10px] tracking-widest pl-8">Doctor Details</TableHead>
                <TableHead className="font-bold text-slate-400 uppercase text-[10px] tracking-widest">Specialization</TableHead>
                <TableHead className="font-bold text-slate-400 uppercase text-[10px] tracking-widest">Location</TableHead>
              ...
              {isLoading ? (
                [...Array(5)].map((_, i) => (
                  <TableRow key={i}>
                    ...
                    <TableCell className="pr-8"><Skeleton className="h-9 w-24 ml-auto rounded-xl" /></TableCell>
                  </TableRow>
                ))
